...
void GameMap::EraseMap(const int& x_pos, const int& y_pos)
{
    int map_x = x_pos/TILE_SIZE;
    int map_y = y_pos/TILE_SIZE;

    if (map_x >= 0 && map_x < MAX_MAP_X - 40 && map_y >= 0 && map_y < MAX_MAP_Y)
    for(int i = map_y; i < 10; i++)
    {
        for(int j = map_x - 2; j <= map_x + 1; j++)
        {
            game_map_.tile[i][j] = BLANK_TILE;
        }
    }
}