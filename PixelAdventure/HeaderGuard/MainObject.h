...
public:
    ...
    void HandelInputAction(SDL_Event events, SDL_Renderer* screen,  Mix_Chunk* bullet_sound[2], Mix_Chunk* jump[1]);
    void set_clips(); //hieu ung animation

    void DoPlayer(Map& map_data, Mix_Chunk* coin[1]);  //
    void CheckToMap(Map& map_data, Mix_Chunk* collect_coin[1]);  //check va cham
    ...
    void IncreaseMoney();
    void UpBlock();
    void DrecreaseBlock() {block_count--;}
    void ChangeBullet();
    ...
    void set_block(int bl) {block_count = bl;}
    void ResetPlay();
private:
